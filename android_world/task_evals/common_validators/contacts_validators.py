...
class AddContact(task_eval.TaskEval):
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    contact_found = expected_contact in actual_contacts
    
    # Debug output when validation fails
    if not contact_found:
      logging.warning(
          'Contact validation failed.\n'
          'Expected: name="%s", number="%s" (cleaned: "%s")\n'
          'Actual contacts found: %s',
          self.params['name'],
          self.params['number'],
          contacts_utils.clean_phone_number(self.params['number']),
          [(c.name, c.number) for c in actual_contacts] if actual_contacts else 'None'
      )

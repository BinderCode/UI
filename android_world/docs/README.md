...
1. Install AndroidWorld. *Note: Python 3.11 or above is required.*
    ...
    If AndroidWorld is the repo root (e.g. after cloning the upstream repo):
    ```bash
    ...
    If AndroidWorld is used as a subdirectory (e.g. `your_project/android_world/`), from your project root:
    ```bash
    pip install -r requirements.txt   # if at project root
    pip install android_world/        # or: cd android_world && pip install .
    ```
...
Run the minimal task runner to see the basic mechanics of AndroidWorld components. It initializes the environment, sets up a task, and runs the default agent, M3A, on it.

From **project root** (ensure `PYTHONPATH` includes the project root so `android_world` and any root-level scripts are importable):
```bash
python minimal_task_runner.py --task=ContactsAddContact
```
If `minimal_task_runner.py` lives inside the `android_world` package, use instead:
```bash
python -m android_world.scripts.minimal_task_runner --task=ContactsAddContact
```
...
```
If `run.py` is inside the `android_world` package, use `python -m android_world.run` with the same flags.
...
2. Import your agent in the benchmark entry script (e.g. `run.py` at project root or `android_world/run.py`) and add it into the `_get_agent` method, which takes your agent's name and returns an instance of it.

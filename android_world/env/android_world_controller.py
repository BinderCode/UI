...
def get_controller(
  ...
  # Check if accessibility forwarder APK is already installed
  # Package name: com.google.androidenv.accessibilityforwarder
  install_a11y_forwarding_app = True
  try:
    packages = adb_utils.get_all_package_names(android_env_instance)
    if 'com.google.androidenv.accessibilityforwarder' in packages:
      logging.info(
          'Accessibility forwarder APK is already installed, skipping installation.'
      )
      install_a11y_forwarding_app = False
  except Exception as e:
    logging.warning(
        'Failed to check if accessibility forwarder is installed: %s. '
        'Will attempt to install.', e
    )
